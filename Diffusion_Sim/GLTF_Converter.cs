...
namespace Diffusion_Sim
{
    class GLTF_Converter
    {
        ...
        public List<GraphicsObject.Section> GetBufferData()
        {
            List<GraphicsObject.Section> sections = new List<GraphicsObject.Section>();

            foreach (Mesh m in ModelData.meshes)
            {
                foreach (Mesh.Primitive p in m.primitives)
                {
                    GraphicsObject.Section section = new GraphicsObject.Section();
